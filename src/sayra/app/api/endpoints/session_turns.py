...
async def list_turns(
    ...
    items, total = await turn_service.list_turns(session_id, offset, limit)
    return TurnListResponse(items=list(items), total=total)


@router.get("/{session_id}/turns/{turn_id}", response_model=TurnResponse)
async def get_turn(session_id: str, turn_id: str):
    return await turn_service.get_turn(session_id, turn_id)
